...
export async function adminSummary() { const db = await getDb(); if (!db) return { users: 0, meals: 0, pantryItems: 0, aiOperations: 0, subscriptions: 0, failedOperations: 0, analyticsEvents: 0 }; const [u, m, p, a, s, f, e] = await Promise.all([db.select({ count: sql<number>`count(*)` }).from(users), db.select({ count: sql<number>`count(*)` }).from(meals), db.select({ count: sql<number>`count(*)` }).from(pantryItems), db.select({ count: sql<number>`count(*)` }).from(usageEvents), db.select({ count: sql<number>`count(*)` }).from(subscriptions), db.select({ count: sql<number>`count(*)` }).from(usageEvents).where(eq(usageEvents.success, false)), db.select({ count: sql<number>`count(*)` }).from(analyticsEvents)]); const aiOperations = Number(a[0]?.count ?? 0); const failedOperations = Number(f[0]?.count ?? 0); const analyticsEventCount = Number(e[0]?.count ?? 0); return { users: Number(u[0]?.count ?? 0), meals: Number(m[0]?.count ?? 0), pantryItems: Number(p[0]?.count ?? 0), aiOperations, subscriptions: Number(s[0]?.count ?? 0), failedOperations, failureRate: aiOperations ? failedOperations / aiOperations : 0, analyticsEvents: analyticsEventCount, mealGenerationConversion: analyticsEventCount ? Number(m[0]?.count ?? 0) / analyticsEventCount : 0 }; }
