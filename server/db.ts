...
export async function adminSummary() { const db = await getDb(); if (!db) return { users: 0, meals: 0, pantryItems: 0, aiOperations: 0 }; const [u, m, p, a] = await Promise.all([db.select({ count: sql<number>`count(*)` }).from(users), db.select({ count: sql<number>`count(*)` }).from(meals), db.select({ count: sql<number>`count(*)` }).from(pantryItems), db.select({ count: sql<number>`count(*)` }).from(usageEvents)]); return { users: Number(u[0]?.count ?? 0), meals: Number(m[0]?.count ?? 0), pantryItems: Number(p[0]?.count ?? 0), aiOperations: Number(a[0]?.count ?? 0) }; }
